...
public class ChefRecommendationRepository<T> implements Repository<T> {
	...
	public int save(List<T> entitiesToSave) throws SQLException {
		...
		for (T recommendedItem : entitiesToSave) {
			...
			int paramIndex = 1;
			if (chefRecommendedItem.getMenuId() > 0)
				statement.setInt(paramIndex++, chefRecommendedItem.getMenuId());
			if (chefRecommendedItem.getVoteCount() > 0)
				statement.setInt(paramIndex++, chefRecommendedItem.getVoteCount());
...
//	             if (newItem.getMenuId() > 0) statement.setInt(paramIndex++, newItem.getMenuId());
	...
	public List<T> findRecords(String query) throws SQLException {
		...
		while (cursor.next()) {
			...
			if (isColumnPresent(rsmd, "VoteCount"))
				retrievedRecord.setVoteCount(cursor.getInt("VoteCount"));
			if (isColumnPresent(rsmd, "vegetarian_preference") && cursor.getString("Preference") != null)
				retrievedRecord.setPreference(cursor.getString("Preference"));
			if (isColumnPresent(rsmd, "spice_level"))
				retrievedRecord.setSpiceLevel(cursor.getString("SpiceLevel"));
			if (isColumnPresent(rsmd, "cuisine_preference"))
				retrievedRecord.setCuisinePreference(cursor.getString("CuisinePreference"));
			if (isColumnPresent(rsmd, "sweet_tooth"))
				retrievedRecord.setSweetTooth(cursor.getString("SweetTooth"));
			if (isColumnPresent(rsmd, "MealType"))
				retrievedRecord.setMealType(cursor.getString("MealType"));
