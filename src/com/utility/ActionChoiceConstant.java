package com.utility;

public class ActionChoiceConstant {

	public static final String AUTHENTICATE_USER = "authenticate_user";
	public static final String ADMIN = "Admin";
	public static final String CHEF = "Chef";
	public static final String EMPLOYEE = "Employee";
	public static final String ADMIN_ADD = "Admin:add";
	public static final String ADMIN_VIEW = "Admin:view";
	public static final String ADMIN_VIEW_RESPONSE = "Admin:viewResponse";
	public static final String ADMIN_UPDATE = "Admin:update";
	public static final String ADMIN_DELETE = "Admin:delete";
	public static final String CHEF_VIEW = "Chef:view";
	public static final String CHEF_VIEW_RECOMMENDATION = "Chef:view_recommendation";
	public static final String CHEF_ROLLOUT_NEXT_DAY_MENU = "Chef:roll_out_next_day_menu";
	public static final String CHEF_VIEW_VOTED_REPORT = "Chef:view_voted_report";
	public static final String CHEF_VIEW_DISCARD_MENU_ITEM_LIST = "Chef:view_discard_menu_item_list";
	public static final String CHEF_VIEW_DISCARD_MENU_ITEM_LIST_RESPONSE = "Chef:view_discard_menu_item_list_response";
	public static final String CHEF_DISCARD_ITEM = "Chef:discard_item";
	public static final String CHEF_GET_DETAILED_FEEDBACK = "Chef:get_detailed_feedback";
	public static final String EMPLOYEE_VIEW_CHEF_RECOMMENDATION = "Employee:view_chefRecommendations";
	public static final String EMPLOYEE_VIEW_CHEF_RECOMMENDATION_RESPONSE = "Employee:view_chefRecommendations_response";
	public static final String EMPLOYEE_VOTE_ITEMS = "Employee:voteItem";
	public static final String EMPLOYEE_FEEDBACK = "Employee:feedback";
	public static final String EMPLOYEE_VIEW_NOTIFICATION = "Employee:view_notification";
	public static final String EMPLOYEE_UPDATE_PROFILE = "Employee:updateProfile";
	public static final String EMPLOYEE_VIEW_MENU = "Employee:view";  

}
